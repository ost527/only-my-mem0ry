...
| 프롬프트 | `answer(query)` | **오직 저장된 메모리만** 근거로 질문에 답함 — 서버가 관련 메모리를 회수·프레이밍하고, 에이전트가 `[id]`를 인용하며 답함(서버 LLM 없는 로컬판 RAG `answer`). |
...
3단계를 쉽게 하도록 `add_memory`는 가장 가까운 기존 메모리를 코사인 유사도와 함께
반환하고, 새 항목이 **거의 중복으로 보이면 경고**합니다(유사도 ≥ `MEM0_DUP_THRESHOLD`) —
그래서 중복을 쌓지 말고 조정(update/병합)하도록 유도합니다. 내부적으로 서버는 mem0의
`infer=False` 경로를 사용합니다 — 임베딩 후 그대로 저장 — 따라서 쓰기는 즉각적이고
결정적이며 모델 호출이 없습니다.
