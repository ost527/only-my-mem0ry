...
**Phase 1 남은 항목**(다음 작업): `provenance`(#9), `confidence`(#10), `temporal` 필터(#7),
전체 `export`(#16). 모두 끝나면 **v0.5.0** 태그.
...
- [ ] Phase 1 — provenance·confidence·temporal 필터·`answer` 프롬프트·export (v0.5.0)
- [ ] Phase 2 — 버전 히스토리(`memory_history`/`restore_memory`) (v0.6.0)
- [ ] Phase 3 — 충돌 후보 탐지 + (측정 후) recency tie-break (v0.7.0)
- [ ] Phase 4 — 파일 인제스트 + 배치 추가 (v0.8.0)

각 PR은 §5의 3단 게이트(ruff·pytest·eval 비회귀)를 통과해야 머지한다.
...
| ② No temporal decay | temporal 필터(P1) + recency tie-break(P3) | 🔜 계획 |
| ③ No provenance | provenance + confidence(P1) | 🔜 계획 |
| ④ Flat memory | 타입형 13범주 | ✅ v0.4.0 |
| ⑤ No writeback (모순 방치) | 버전 히스토리(P2) + 충돌 후보(P3) | 🔜 계획 |
| ⑥ Indexing delay | `infer=False` 즉시 검색 | ✅ 보유 |

→ 이 로드맵을 완주하면 memanto가 지적한 6가지 격차를 **전부**, 그러나 "클라이언트가 두뇌·
100% 로컬·결정적"이라는 우리 방식으로 메운다.
